...
export default function Home() {
  ...
  const assignedSlots = useMemo(
    () => assignments.filter((a) => a.packerPickerName).length,
    [assignments]
  );
  ...
  const handleRemoveSupervisor = async (supervisorId: string): Promise<void> => {
  ...
  const handleUpdateAssignment = async (updatedAssignment: Partial<Assignment> & { id: string }): Promise<void> => {
    setAssignments((prev) => {
      const newAssignments = prev.map((a) =>
  ...
  const handleClearAssignment = async (assignmentId: string): Promise<void> => {
    setAssignments((prev) => {
      const newAssignments = prev.map((a) =>
    ...
    <main className="flex min-h-screen w-full flex-col">
      <div className="container mx-auto px-4 py-8">
        ...
        <p className="text-muted-foreground mb-8">
          Real-Time Packer/Picker Assignment Registry
        </p>

        <KpiCards
          totalSlots={CTPACKER_SLOTS}
